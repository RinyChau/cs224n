...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        mask = [True] * len(sentence) + [False] * (max_length - len(sentence)) if max_length > len(sentence) else [True] * max_length
        sentence = sentence[:max_length] if max_length <= len(sentence) else sentence + [zero_vector] * (max_length - len(sentence))
        labels = labels[:max_length] if max_length <= len(labels) else labels + [zero_label] * (max_length - len(labels))
...
class RNNModel(NERModel):
    ...
    def create_feed_dict(self, inputs_batch, mask_batch, labels_batch=None, dropout=1):
        ...
        feed_dict = {self.input_placeholder: inputs_batch, self.labels_placeholder: labels_batch, self.dropout_placeholder: dropout, self.mask_placeholder:mask_batch}
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable('b_2', [self.config.n_classes], tf.float32, tf.constant_initializer(0))
        state = tf.zeros([tf.shape(self.input_placeholder)[0], self.config.hidden_size], tf.float32)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                y_t = tf.matmul(dropout_output, U) + b2
                preds.append(y_t)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        total_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.boolean_mask(self.labels_placeholder, self.mask_placeholder), logits=tf.boolean_mask(preds, self.mask_placeholder))
