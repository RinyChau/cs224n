...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(lookup, (-1, Config.n_features * Config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        xavier_initer = xavier_weight_init()
        W = xavier_initer((Config.n_features*Config.embed_size, Config.hidden_size))
        U = xavier_initer((Config.hidden_size, Config.n_classes))
        b1 = tf.Variable(tf.zeros(Config.hidden_size))
        b2 = tf.Variable(tf.zeros(Config.n_classes))
        h = tf.maximum(tf.matmul(x, W) + b1, 0)
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        batch_loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
