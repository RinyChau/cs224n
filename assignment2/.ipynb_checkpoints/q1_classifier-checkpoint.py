...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W = tf.Variable(np.zeros((self.config.n_features, self.config.n_classes), dtype=np.float32), name='W')
        b = tf.Variable(np.zeros(self.config.n_classes, dtype=np.float32), name='b')
